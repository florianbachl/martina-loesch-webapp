<template>
  <div id="home">
    <header class="header d-md-flex flex-row justify-content-center align-items-center">
      <div class="container col-10 h-100 row">
        
          <div class="col-7">
            <h1 class="h1">Lorem ipsum dolor sit amet, consetetur sadipscing elitr</h1>
            <p class="lead-4 my-4">
              Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam
              nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam
              erat.
            </p>
            <router-link to="/contact">
            <button class="btn btn-primary mt-4" type="button">
              Jetzt Kontaktieren
            </button></router-link>
          </div>
          <div class="col"></div>
       
      </div>
    </header>
    <section class="section">
      <div class="container py-5 col-7">
        <div class="row mt-5">
          <h2 class="h3 text-center">
            Lorem ipsum dolor sit amet, consetetur sadipscing elitr lorem ipsum
            dolor sit amet, consetetur elitr
          </h2>
        </div>
      </div>
    </section>
    <section class="section py-5" id="kinesiologie">
      <div class="container py-4  col">
        <div
          class="d-md-flex flex-row justify-content-center align-items-center"
        >
          <div class="col mx-md-5 px-md-4">
            <img
              src="../assets/sections/Kinesiologie.jpg"
              class="w-100 rounded"
            />
          </div>
          <div class="col mx-md-5 px-md-4 my-4 my-md-0">
            <h2 class="h1">Kranio</h2>
            <p class="lead-4 my-4">
              Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam
              nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam
              erat. Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed
              diam nonumy eirmod tempor invidunt ut labore et dolore magna
              aliquyam erat. Lorem ipsum dolor sit amet, consetetur sadipscing
              elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore
              magna aliquyam erat
            </p>
            <router-link to="/contact" class="link-primary text-uppercase">Primary link</router-link>
            
          </div>
        </div>
      </div>
    </section>
    <section class="section my-5 py-5" id="cranio">
      <div class="container py-4 my-3 col">
        <div
          class="d-md-flex flex-row justify-content-center align-items-center"
        >
          <div class="col mx-md-5 px-md-4 my-4 my-md-0">
            <h2 class="h1">Kranio</h2>
            <p class="lead-4 my-4">
              Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam
              nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam
              erat. Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed
              diam nonumy eirmod tempor invidunt ut labore et dolore magna
              aliquyam erat. Lorem ipsum dolor sit amet, consetetur sadipscing
              elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore
              magna aliquyam erat
            </p>
            <router-link to="/contact" class="link-primary text-uppercase">Primary link</router-link>
          </div>
          <div class="col mx-md-5 px-md-4">
            <img
              src="../assets/sections/Kinesiologie.jpg"
              class="w-100 rounded"
            />
          </div>
        </div>
      </div>
    </section>
    <section class="section my-5 py-5 bg-grey" id="behandlung">
      <div class="container py-5 my-5 col">
        <div
          class=" d-md-flex flex-row justify-content-center align-items-center"
        >
          <div class="col mx-md-5 px-md-4">
            <img
              src="../assets/sections/Kinesiologie.jpg"
              class="w-100 rounded"
            />
          </div>
          <div class="col mx-md-5 px-md-4 my-4 my-md-0">
            <h2 class="h1">Kranio</h2>
            <p class="lead-4 my-4">
              Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam
              nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam
              erat. Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed
              diam nonumy eirmod tempor invidunt ut labore et dolore magna
              aliquyam erat. Lorem ipsum dolor sit amet, consetetur sadipscing
              elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore
              magna aliquyam erat
            </p>
            <h3 class="h4 mt-5">Tarife</h3>
            <div
              class="d-flex flex-row justify-content-between align-items-start"
            >
              <p>Kranio</p>
              <p>12,00€</p>
            </div>
            <div
              class="d-flex flex-row justify-content-between align-items-start"
            >
              <p>Kranio</p>
              <p>12,00€</p>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="section my-5 py-5">
      <div class="container py-5 my-5 mt-0 col">
        <h2 class="h1 text-center py-5">Lorem ipsum</h2>

        <div class="row mt-5">
          <div class="col-1">
            <div class="row">
              <div class="col my-1"><hr /></div>
            </div>
          </div>
          <div class="col row">
            <div class="col">
              <div
                class="d-md-flex flex-row justify-content-center align-items-center"
              >
                <div class="col-auto">
                  <div
                    class="icon-wrapping icon-wrapping-chat d-flex flex-row justify-content-center align-items-center"
                  >
                    <span class="material-icons material-icons-chat"
                      >question_answer</span
                    >
                  </div>
                </div>
                <div class="col"><hr /></div>
              </div>

              <h3 class="h5 my-3">Kranio</h3>
              <p class="lead-4">
                Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed
                diam nonumy eirmod tempor invidunt ut labore et dolore magna
                aliquyam erat.
              </p>
            </div>
            <div class="col">
              <div
                class="d-md-flex flex-row justify-content-center align-items-center"
              >
                <div class="col-auto">
                  <div
                    class="icon-wrapping icon-wrapping-wave d-flex flex-row justify-content-center align-items-center"
                  >
                    <span class="material-icons material-icons-wave"
                      >waving_hand</span
                    >
                  </div>
                </div>
                <div class="col"><hr /></div>
              </div>
              <h3 class="h5 my-3">Kranio</h3>
              <p class="lead-4">
                Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed
                diam nonumy eirmod tempor invidunt ut labore et dolore magna
                aliquyam erat.
              </p>
            </div>
            <div class="col">
              <div
                class="d-md-flex flex-row justify-content-center align-items-center"
              >
                <div class="col-auto">
                  <div
                    class="icon-wrapping icon-wrapping-smile d-flex flex-row justify-content-center align-items-center"
                  >
                    <span class="material-icons material-icons-smile"
                      >mood</span
                    >
                  </div>
                </div>
                <div class="col"><hr /></div>
              </div>

              <h3 class="h5 my-3">Kranio</h3>
              <p class="lead-4">
                Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed
                diam nonumy eirmod tempor invidunt ut labore et dolore magna
                aliquyam erat.
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="section my-5 py-5 bg-grey" id="about">
      <div class="container my-5 py-5 col-7">
        <div
          class="d-md-flex flex-row justify-content-center align-items-start"
        >
          <div class="col-8 mx-md-5 px-md-5 my-4 my-md-0">
            <h2 class="h1">Kranio</h2>
            <p class="lead-4 my-4">
              Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam
              nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam
              erat.Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed
              diam nonumy eirmod tempor invidunt ut labore et dolore magna
              aliquyam erat.Lorem ipsum dolor sit amet, consetetur sadipscing
              elitr, sed diam nonumy eirmod tempor invidunt ut
              <br />
              <br />
              labore et dolore magna aliquyam erat.Lorem ipsum dolor sit amet,
              consetetur sadipscing elitr, sed diam nonumy eirmod tempor
              invidunt ut labore et dolore magna aliquyam erat.Lorem ipsum dolor
              sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod
              tempor invidunt ut labore et dolore magna aliquyam erat.Lorem
              ipsum dolor sit amet, consetetur sadipscing elitr, sed

              <br />
              <br />
              diam nonumy eirmod tempor invidunt ut labore et dolore magna
              aliquyam erat.Lorem ipsum dolor sit amet, consetetur sadipscing
              elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore
              magna aliquyam erat.Lorem ipsum dolor sit amet.
            </p>
            <h3 class="h4 mt-5">Meine Ausbildungen</h3>
            <div
              class="d-flex flex-row justify-content-start align-items-center mt-4"
            >
              <div class="col-auto">
                <div
                  class="icon-wrapping icon-wrapping-achievement d-flex flex-row justify-content-center align-items-center"
                >
                  <span class="material-icons material-icons-chat achievement"
                    >military_tech</span
                  >
                </div>
              </div>
              <div class="col h6">Lorem Ipsum Dolor sit Amet</div>
            </div>
            <div
              class="d-flex flex-row justify-content-start align-items-center mt-3"
            >
              <div class="col-auto">
                <div
                  class="icon-wrapping icon-wrapping-achievement d-flex flex-row justify-content-center align-items-center"
                >
                  <span class="material-icons material-icons-chat achievement"
                    >military_tech</span
                  >
                </div>
              </div>
              <div class="col h6">Lorem Ipsum Dolor sit Amet</div>
            </div>
            <div class="mt-3">
              <router-link to="/contact">
              <button class="btn btn-primary mt-5" type="button">
                Jetzt Kontaktieren
              </button>
              </router-link>
            </div>
          </div>
          <div class="col-4 mx-md-5 px-md-5">
            <img src="../assets/sections/portrait.jpg" class="w-100 rounded" />
          </div>
        </div>
      </div>
    </section>
    <section class="section my-5 py-5">
      <div class="container my-5 py-5 mt-0 col-7">
        <h2 class="h1 text-center py-5">Lorem ipsum</h2>
        <p class="lead-4">
          Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam
          nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat.
          Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam
          nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat.
          Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam
          nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat
        </p>
        <div class="row mt-5">
          <div class="col">
            <div class="row mt-4">
              <div class="col-auto">
                <span class="material-icons">arrow_circle_right</span>
              </div>
              <div class="col">
                <p class="h5">
                  Lorem ipsum dolor sit amet, consetetur sadipscing elitr.
                </p>
              </div>
            </div>
            <div class="row mt-4">
              <div class="col-auto">
                <span class="material-icons">arrow_circle_right</span>
              </div>
              <div class="col">
                <p class="h5">
                  Lorem ipsum dolor sit amet, consetetur sadipscing elitr.
                </p>
              </div>
            </div>
            <div class="row mt-4">
              <div class="col-auto">
                <span class="material-icons">arrow_circle_right</span>
              </div>
              <div class="col">
                <p class="h5">
                  Lorem ipsum dolor sit amet, consetetur sadipscing elitr.
                </p>
              </div>
            </div>
          </div>
          <div class="col">
            <div class="row mt-4">
              <div class="col-auto">
                <span class="material-icons">arrow_circle_right</span>
              </div>
              <div class="col">
                <p class="h5">
                  Lorem ipsum dolor sit amet, consetetur sadipscing elitr.
                </p>
              </div>
            </div>
            <div class="row mt-4">
              <div class="col-auto">
                <span class="material-icons">arrow_circle_right</span>
              </div>
              <div class="col">
                <p class="h5">
                  Lorem ipsum dolor sit amet, consetetur sadipscing elitr.
                </p>
              </div>
            </div>
            <div class="row mt-4">
              <div class="col-auto">
                <span class="material-icons">arrow_circle_right</span>
              </div>
              <div class="col">
                <p class="h5">
                  Lorem ipsum dolor sit amet, consetetur sadipscing elitr.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="section bg-gradient-new">
     <div class="container py-5 col">
        <div
          class="d-md-flex flex-row justify-content-center align-items-center"
        >
          <div class="col-7 py-5">
            <h1 class="h1 text-white">Sed diam
              nonumy eirmod tempor invidunt ut labore</h1>
            <p class="h5 my-4 text-white">
              Lorem ipsum dolor sit amet, consetetur sadipscing elitr
            </p>
            <router-link to="/contact">
            <button class="btn btn-secondary-white mt-4" type="button">
              Jetzt Kontaktieren
            </button></router-link>
          </div>
          <div class="col"></div>
        </div>
      </div>
    </section>
  </div>
</template>

<script>
export default {
  name: "home-comp",
  data() {
    return {};
  },
  components: {},
  computed: {},
  watch: {},
  methods: {},
  mounted() {
    //window.scrollTo(0, 0);
  },
};
</script>
<style scoped>
.icon-wrapping {
  padding: 10px;
  border-radius: 10px;
  margin-right: 24px;
}

.icon-wrapping-chat {
  background: rgb(224, 218, 51);
  background: linear-gradient(
    234deg,
    rgba(224, 218, 51, 0.10968137254901966) 0%,
    rgba(254, 136, 1, 0.10968137254901966) 100%
  );
}

.icon-wrapping-achievement {
  background: white;
}

.icon-wrapping-wave {
  background: rgb(64, 36, 255);
  background: linear-gradient(
    234deg,
    rgba(64, 36, 255, 0.10688025210084029) 0%,
    rgba(62, 222, 255, 0.10688025210084029) 100%
  );
}

.icon-wrapping-smile {
  background: rgb(255, 0, 136);
  background: linear-gradient(
    234deg,
    rgba(255, 0, 136, 0.10688025210084029) 0%,
    rgba(255, 170, 255, 0.11248249299719892) 100%
  );
}

.icon-wrapping .material-icons {
  -webkit-background-clip: text;
  -moz-background-clip: text;
  background-clip: text;
  -webkit-text-fill-color: transparent;
}
.material-icons-chat {
  background: -moz-linear-gradient(top, #e0da33 0%, #fe8801 100%);
  background: -webkit-linear-gradient(top, #e0da33 0%, #fe8801 100%);
  background: linear-gradient(to bottom, #e0da33 0%, #fe8801 100%);
}

.material-icons-wave {
  background: -moz-linear-gradient(top, #4024ff 0%, #3edeff 100%);
  background: -webkit-linear-gradient(top, #4024ff 0%, #3edeff 100%);
  background: linear-gradient(to bottom, #4024ff 0%, #3edeff 100%);
}

.material-icons-smile {
  background: -moz-linear-gradient(top, #ff0088 0%, #ffaaff 100%);
  background: -webkit-linear-gradient(top, #ff0088 0%, #ffaaff 100%);
  background: linear-gradient(to bottom, #ff0088 0%, #ffaaff 100%);
}

.achievement {
  font-size: 20px;
}
</style>
